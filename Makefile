...
tmpnb: tutorial-image
	docker run --net=host -d -e CONFIGPROXY_AUTH_TOKEN=devtoken \
		...
		--command="ipython notebook --NotebookApp.base_url={base_path} --ip=0.0.0.0 --port {port} --NotebookApp.server_extensions=\"['extensions.start_mongo', 'extensions.start_supervisord']\""
